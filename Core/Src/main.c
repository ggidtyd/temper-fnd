...
static void MX_GPIO_Init(void) {
	...
	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(PC8_TEMP_DATA_GPIO_Port, PC8_TEMP_DATA_Pin, GPIO_PIN_SET);
	...
	/*Configure GPIO pin : PC8_TEMP_DATA_Pin */
	GPIO_InitStruct.Pin = PC8_TEMP_DATA_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_PULLUP;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
	HAL_GPIO_Init(PC8_TEMP_DATA_GPIO_Port, &GPIO_InitStruct);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
	if (htim->Instance == TIM3) {
		if (isTemperSensorInit() && !isBusy())
			digit4_show((int) (tempersensor.Temperature * 10));
	}
}
/* USER CODE END 4 */
